...
export class Reminder extends React.Component<IReminderProps> {
  componentDidMount() {
    this.props.getEntities();
  }

  render() {
    const { reminderList, match } = this.props;
    return (
      <div>
        <h2 id="reminder-heading">
          <Translate contentKey="taskMasterApp.reminder.home.title">Reminders</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="taskMasterApp.reminder.home.createLabel">Create new Reminder</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {reminderList.map((reminder, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={reminder.reminderDate} format={APP_DATE_FORMAT} />
                  </td>
                  <td>{reminder.task ? <Link to={`task/${reminder.task.id}`}>{reminder.task.id}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`${match.url}/${reminder.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${reminder.id}/delete`} color="danger" size="sm">
